...
| Thiel's Secret | An important truth very few people agree with you on — and that, by definition, you can't yet prove. Used here as a hypothesis generator, not a truth gate: each Secret produces a venture concept, the one assumption that must hold (its load-bearing hypothesis), and the cheapest test to check it. You decide which experiments to fund, not which secrets are true. |
| Capability seed (Move 8) | A job that becomes *possible* — not just cheaper — because a new capability now exists. The first-principles lane: instead of "who do we attack?", it asks "what is now possible that couldn't exist before?" No incumbent required. |
| Bet (load-bearing hypothesis + validation test) | Every concept ships as a bet: the single claim that, if false, kills it, plus the cheapest experiment that would tell you whether it holds. The gate decision is which bets are worth testing. |
