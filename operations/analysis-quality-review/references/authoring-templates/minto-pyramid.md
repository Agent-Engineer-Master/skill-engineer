...
>
> **How to use this template.** You — the calling skill — fill this in using your subject context BEFORE writing the document. The goal is the ghost-deck discipline: fix the storyline FIRST, then write the prose. When this template is filled in, the result is a tailored authoring spec for your specific subject. You then submit that spec to `analysis-quality-review` in `mode: spec-judge` for grading. If the judge returns PASS, you write the document. If FAIL, you fix the spec (not the document — there is no document yet) and re-submit. After 3 failed iterations, the judge returns ESCALATE — consult a human.
>
