...
- `template_path` — the authoring template the spec was filled from (e.g., `.claude/skills/write-report/references/authoring-templates/brief.md`)
...
1. **Rubric (trusted policy):** `Read .claude/skills/write-report/references/spec-judge-rubric.md`
...
5. **Worked examples (reference only):** `Read .claude/skills/write-report/references/worked-examples.md` when interpretation is ambiguous.
...
Spec-judge is the grader for the spec-review loop. The orchestrator (`write-report` SKILL.md, `mode: spec-judge`) will:
