...
1. **Rubric (trusted policy):** `Read .claude/skills/write-report/references/rubric-structure.md`
...
5. **Worked examples (reference only):** `Read .claude/skills/write-report/references/worked-examples.md` only when a dimension's interpretation is ambiguous
...
This agent is the grader for Pass 1. The orchestrator (`write-report` SKILL.md) will:
